module.exports = function(RED) {
  ...
  var LightScheduler = function(n) {
    ...
    function setState(out) {
      ...
      // this.log("msg: " + msg.payload.toString + " prePayload: " + node.prevPayload.toString);
      // var debugMsg = {
      //   newMsg: msg.payload,
      //   oldMsg: node.prevPayload,
      //   compare: (msg.payload === node.prevPayload)
      // }
      // node.send(debugMsg);
      if(node.outputfreq == 'output.minutely' || node.prevPayload === null || msg.payload.on !== node.prevPayload.on)
      {
        node.send(msg);
